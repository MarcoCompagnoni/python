...
# GET CONFIGURAZIONI THRESHOLD ####
    select_conf_threshold = "SELECT tipologia_controllo, impianto, apparecchiatura, strumento, risoluzione_temporale," \
                            " from_date, to_date, delta FROM RAM_ANACONDA_APP_VIBRA_CONF_THRESHOLD ORDER BY " \
...
# CALCOLO SOGLIA ####
        select_app_vibra_soglia = "SELECT tipologia_controllo, impianto, apparecchiatura, strumento, avg(value) AS " \
            "media_valori FROM VW_RAM_APP_VIBRA WHERE tipologia_controllo LIKE ? AND impianto LIKE ? AND " \
                                  "apparecchiatura LIKE ? AND strumento LIKE ? AND risoluzione_temporale=? AND value " \
                                  ">= value_min AND timestamp BETWEEN ? AND ? GROUP BY tipologia_controllo, impianto," \
