...
def analytics(conf_analytics):
    ...
    for i in range(0, number_of_rows):
        ...
        if first_time_populate_db:

            # TODO <parametro> popolare con quanti giorni >= time_win
            quanti_giorni_andiamo_indietro = int(time_win)
            time_start_analytics = (today - timedelta(days=quanti_giorni_andiamo_indietro))
            ...
            if tipo_calcolo == "mavg" and (calcolo == "sma" or calcolo == "wma"):
                ...
                time_start_query = time_start_analytics

        else:

            time_start_analytics = (today - timedelta(days=time_win))
            time_start_query = time_start_analytics

        values = [time_start_query.strftime("%Y-%m-%d"), risoluzione_temporale]
        print("eseguo il calcolo " + tipo_calcolo + " (" + calcolo + ") dal giorno " + time_start_analytics.strftime(
            "%Y-%m-%d"))
            ...
            for strumento in strumenti:
                ...
                if tipo_media_mobile == "sma":
                    ...
                    if first_time_populate_db:
                        ...
                        window = len(dataframe_strumento.index)
